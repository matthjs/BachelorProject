...
from bachelorproject.models.trainers.trainer import AbstractTrainer
...
class GPQTrainer(AbstractTrainer):
    ...
    def _construct_target_dataset(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(states.shape) == 1:  # Check if shape is [batch_size]
            states = states.unsqueeze(1)  # Convert shape to [batch_size, 1]

        if len(next_states.shape) == 1:  # Check if shape is [batch_size]
            next_states = next_states.unsqueeze(1)  # Convert shape to [batch_size, 1]
